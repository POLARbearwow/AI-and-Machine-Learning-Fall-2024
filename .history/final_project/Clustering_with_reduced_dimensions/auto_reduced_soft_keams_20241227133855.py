...
class Soft_K_means:
    def fit(self, X, K=5, beta=3, max_iters=100):
        ...
        # initialize mean of clusters using K-Means++ to get initial centroids
...
# 运行 Soft K-Means 聚类
s = Soft_K_means()
centroids, responsibilities = s.fit(autoencoded_features, K=k, beta=10)

# 将每个样本分配到概率最大的簇
labels_autoencoder = np.argmax(responsibilities, axis=1)
...
plt.title('Soft K-Means on Autoencoder Features')
