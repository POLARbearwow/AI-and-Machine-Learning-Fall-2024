...
# ----------------------------- 使用 OneVsRestClassifier 包装 BiLinearSVC ----------------------------- #
linear_svc = OneVsRestClassifier(BiLinearSVC(C=1.0, max_iter=1000, tol=1e-5))  # 使用 OvR 策略
...
print("BiLinearSVC Model Evaluation with OvR:")
...
# ----------------------------- 使用 OneVsRestClassifier 包装 BiKernelSVC ----------------------------- #
kernel_svc = OneVsRestClassifier(BiKernelSVC(C=1.0, kernel='rbf', max_iter=1000, tol=1e-5))  # 使用 OvR 策略
...
print("\nBiKernelSVC Model Evaluation with OvR:")
...
# 仅可视化前两个特征
plot_decision_boundary(X_train, y_train, linear_svc, 'BiLinearSVC Decision Boundary with OvR')
plot_decision_boundary(X_train, y_train, kernel_svc, 'BiKernelSVC Decision Boundary with OvR')
