...
# 3. 将 class_label 转换为 0, 2 的格式 (class的label数字影响？ 必须有个0？)
...
# 14. 绘制损失函数和测试集评估指标的变化
fig, ax = plt.subplots(1, 2, figsize=(14, 6))

# 子图 1: 绘制损失函数
ax[0].plot(range(1, len(adaboost.losses) + 1), adaboost.losses, color='blue', linewidth=2, marker='o', label='Training Loss')
ax[0].set_xlabel('Number of Estimators (T)')
ax[0].set_ylabel('Loss')
ax[0].set_title('MLP Loss Curve')
ax[0].legend()
ax[0].grid()
...
colors = ['skyblue', 'limegreen', 'orange', 'tomato']  # 自定义颜色

# 绘制柱状图
bars = ax[1].bar(labels, test_scores, color=colors, edgecolor='black')
